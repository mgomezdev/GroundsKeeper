export default {
  ...
  settings: {
    ...
    ldap: {
      ...
      feature3: '登录时 LDAP 组映射到 GroundsKeeper 组',
    ...
    authDisabledMessage: '启用身份验证以创建用户账户、管理权限并保护您的 GroundsKeeper 实例。',
    ...
    disableAuthenticationMessage: '确定要禁用身份验证吗？这将使您的 GroundsKeeper 实例无需登录即可访问。所有用户将保留在数据库中但身份验证将被禁用。',
    disableAuthentication: '禁用身份验证',
    // Additional settings
    configureGroundsKeeper: '配置 GroundsKeeper',
    systemDefault: '系统默认',
    ...
    bambuddySoftware: 'GroundsKeeper 软件',
    autoCheckDescription: '启动时自动检查新版本',
    checkNow: '立即检查',
    updateAvailableVersion: '可用更新：v{{version}}',
    releaseNotes: '发布说明',
    updateViaDocker: '通过 Docker Compose 更新：',
    updateViaHomeAssistant: '更新由 Home Assistant Supervisor 管理。请在 Home Assistant 中打开 设置 → 加载项 → GroundsKeeper 以安装新版本。',
    installUpdate: '安装更新',
    ...
    goToBackup: '前往备份',
    externalUrl: '外部URL',
    externalUrlDescription: 'GroundsKeeper可访问的外部URL。用于通知图像和外部集成。',
    ...
    mqttDescription: '将GroundsKeeper事件发布到外部MQTT代理，用于与Node-RED、Home Assistant和其他自动化系统集成。',
    ...
    encryption: {
      ...
      migrationErrorWarning: '{{count}} 行旧数据在启动时未能重新加密。请检查服务器日志并重启 GroundsKeeper 以重试。',
    },
  ...
  login: {
    ...
    howToReset: '如何重置密码：',
    resetStep1: '联系您的 GroundsKeeper 管理员',
    ...
    twoFA: {
      ...
      methodEmail: '邮箱验证码',
      methodBackup: '备用恢复码',
      instructionsTotp: '请打开您的身份验证器应用，输入 GroundsKeeper 的 6 位验证码。',
  ...
  // Setup page
  setup: {
    title: 'GroundsKeeper 设置',
    subtitle: '为您的 GroundsKeeper 实例配置身份验证',
    enableAuth: '启用身份验证',
    adminAccount: '管理员账户',
  ...
  users: {
    title: '用户管理',
    subtitle: '管理用户及其对 GroundsKeeper 实例的访问',
    backToSettings: '返回设置',
    createUser: '创建用户',
    noPermission: '您没有访问此页面的权限。',
    admin: '管理员',
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'GroundsKeeper 通过您的反向代理以 HTTPS 提供服务，但您的 Spoolman 地址仍为 HTTP。出于安全考虑，浏览器会阻止混合内容，因此嵌入式 Spoolman 界面无法加载。Spoolman 也必须通过 HTTPS 访问。',
    spoolmanMixedContentFixReverseProxy: '请将 Spoolman 置于与 GroundsKeeper 相同的反向代理（Traefik / Nginx / Caddy）之后并启用 HTTPS，然后在设置中将 Spoolman URL 更新为新的 HTTPS 地址。',
  ...
  backup: {
    ...
    appSettings: '应用设置',
    appSettingsDescription: 'GroundsKeeper 配置（完整数据库）',
    spoolInventory: '耗材库存',
    ...
    // Local Backup
    localBackup: '本地备份',
    localBackupDescription: '创建 GroundsKeeper 数据的完整备份，包括数据库、档案、上传和所有文件。',
    ...
    backupRestoredRestart: '备份已恢复。请重启 GroundsKeeper。',
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: '备注保存在 GroundsKeeper 中，不在打印机上',
      syncing: '与打印机同步中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '启用虚拟打印机',
      visibleInSlicer: '在切片软件发现中显示为"GroundsKeeper"',
      proxyingTo: '代理到 {{name}}',
      notActive: '未激活',
    },
    model: {
    ...
    remoteInterface: {
      ...
      placeholder: '自动检测（默认）...',
      hint: '覆盖通过 SSDP 广播并在 TLS 证书中使用的 IP 地址。在 GroundsKeeper 有多个网络接口时很有用。',
    },
    mode: {
      title: '模式',
      archive: '归档',
    ...
    howItWorks: {
      ...
      step2: '在归档、审核和队列模式下，使用切片软件中的"发送"按钮将 3MF 文件上传到 GroundsKeeper。切片软件会显示"打印成功"— 文件已存储，未打印。',
  ...
  spoolbuddy: {
    ...
    settings: {
      ...
      uptime: '运行时间',
      systemConfig: '后端与认证',
      backendUrl: 'GroundsKeeper 后端 URL',
      apiToken: 'API 令牌',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: '粘贴 MakerWorld 模型链接，即可直接在 GroundsKeeper 中导入并打印 —— 无需切换到 Bambu Handy 应用。',
    ...
    disclaimer: 'MakerWorld 集成使用由社区记录的 API 接口。GroundsKeeper 与 MakerWorld 或 Bambu Lab 没有从属或认可关系。',
  ...
  libraryPurge: {
    ...
    effect4: 'Files in external (linked) folders are skipped — GroundsKeeper never deletes bytes it does not own.',    previewLoading: '正在检查匹配的文件数量…',
