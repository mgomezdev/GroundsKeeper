export default {
  ...
  settings: {
    ...
    ldap: {
      ...
      feature3: '登入時 LDAP 群組對應到 GroundsKeeper 群組',
    ...
    authDisabledMessage: '啟用身份驗證以建立使用者帳戶、管理權限並保護您的 GroundsKeeper 實例。',
    ...
    disableAuthenticationMessage: '確定要停用身份驗證嗎？這將使您的 GroundsKeeper 實例無需登入即可存取。所有使用者將保留在資料庫中但身份驗證將被停用。',
    disableAuthentication: '停用身份驗證',
    // Additional settings
    configureGroundsKeeper: '設定 GroundsKeeper',
    systemDefault: '系統預設',
    ...
    bambuddySoftware: 'GroundsKeeper 軟體',
    autoCheckDescription: '啟動時自動檢查新版本',
    checkNow: '立即檢查',
    updateAvailableVersion: '可用更新：v{{version}}',
    releaseNotes: '發布說明',
    updateViaDocker: '透過 Docker Compose 更新：',
    updateViaHomeAssistant: '更新由 Home Assistant Supervisor 管理。請在 Home Assistant 中開啟 設定 → 附加元件 → GroundsKeeper 以安裝新版本。',
    installUpdate: '安裝更新',
    ...
    goToBackup: '前往備份',
    externalUrl: '外部URL',
    externalUrlDescription: 'GroundsKeeper可存取的外部URL。用於通知影像和外部整合。',
    ...
    mqttDescription: '將GroundsKeeper事件發布到外部MQTT代理，用於與Node-RED、Home Assistant和其他自動化系統整合。',
    ...
    encryption: {
      ...
      migrationErrorWarning: '{{count}} 行舊資料在啟動時未能重新加密。請檢查伺服器日誌並重新啟動 GroundsKeeper 以重試。',
    },
  ...
  login: {
    ...
    howToReset: '如何重設密碼：',
    resetStep1: '聯絡您的 GroundsKeeper 管理員',
    ...
    twoFA: {
      ...
      methodEmail: '信箱驗證碼',
      methodBackup: '備用恢復碼',
      instructionsTotp: '請開啟您的身份驗證器 App，輸入 GroundsKeeper 的 6 位驗證碼。',
  ...
  // Setup page
  setup: {
    title: 'GroundsKeeper 設定',
    subtitle: '為您的 GroundsKeeper 實例設定身份驗證',
    enableAuth: '啟用身份驗證',
    adminAccount: '管理員帳戶',
  ...
  users: {
    title: '使用者管理',
    subtitle: '管理使用者及其對 GroundsKeeper 實例的存取',
    backToSettings: '返回設定',
    createUser: '建立使用者',
    noPermission: '您沒有存取此頁面的權限。',
    admin: '管理員',
  ...
  inventory: {
    ...
    spoolmanMixedContentBody: 'GroundsKeeper 透過您的反向代理以 HTTPS 提供服務，但您的 Spoolman 位址仍為 HTTP。基於安全考量，瀏覽器會封鎖混合內容，因此內嵌的 Spoolman 介面無法載入。Spoolman 也必須可透過 HTTPS 存取。',
    spoolmanMixedContentFixReverseProxy: '請將 Spoolman 置於與 GroundsKeeper 相同的反向代理（Traefik / Nginx / Caddy）之後並啟用 HTTPS，然後在設定中將 Spoolman URL 更新為新的 HTTPS 位址。',
  ...
  backup: {
    ...
    appSettings: '應用程式設定',
    appSettingsDescription: 'GroundsKeeper 設定（完整資料庫）',
    spoolInventory: '耗材庫存',
    ...
    // Local Backup
    localBackup: '本機備份',
    localBackupDescription: '建立 GroundsKeeper 資料的完整備份，包括資料庫、檔案、上傳和所有檔案。',
    ...
    backupRestoredRestart: '備份已恢復。請重新啟動 GroundsKeeper。',
  ...
  kProfiles: {
    ...
    modal: {
      ...
      notesHelp: '備註儲存在 GroundsKeeper 中，不在印表機上',
      syncing: '與印表機同步中...',
  ...
  virtualPrinter: {
    ...
    enable: {
      title: '啟用虛擬印表機',
      visibleInSlicer: '在切片軟體發現中顯示為"GroundsKeeper"',
      proxyingTo: '代理到 {{name}}',
      notActive: '未啟用',
    },
    model: {
    ...
    remoteInterface: {
      ...
      placeholder: '自動檢測（預設）...',
      hint: '覆蓋透過 SSDP 廣播並在 TLS 憑證中使用的 IP 位址。在 GroundsKeeper 有多個網路介面時很有用。',
    },
    mode: {
      title: '模式',
      archive: '歸檔',
    ...
    howItWorks: {
      ...
      step2: '在歸檔、審核和佇列模式下，使用切片軟體中的"傳送"按鈕將 3MF 檔案上傳到 GroundsKeeper。切片軟體會顯示"列印成功"— 檔案已儲存，未列印。',
  ...
  spoolbuddy: {
    ...
    settings: {
      ...
      uptime: '執行時間',
      systemConfig: '後端與認證',
      backendUrl: 'GroundsKeeper 後端 URL',
      apiToken: 'API 權杖',
  ...
  failureDetection: {
    ...
    externalUrlHint: 'ML API 透過 URL 擷取攝影機快照。請在一般設定中設定外部 URL，讓 ML API 容器可以連線到 GroundsKeeper。',
  ...
  makerworld: {
    title: 'MakerWorld',
    description: '貼上 MakerWorld 模型連結，即可直接在 GroundsKeeper 中匯入並列印 —— 無需切換至 Bambu Handy 應用程式。',
    ...
    disclaimer: 'MakerWorld 整合使用由社群記錄的 API 介面。GroundsKeeper 與 MakerWorld 或 Bambu Lab 無從屬或認可關係。',
  ...
  libraryPurge: {
    ...
    effect4: 'Files in external (linked) folders are skipped — GroundsKeeper never deletes bytes it does not own.',    previewLoading: '正在檢查符合的檔案數量…',
